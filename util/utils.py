...
# utils.py 作用总结：提供工具函数支持 HGAT 模型训练和图操作
# - sum_up_params：计算模型参数总数，辅助调试。
# - add_reverse_edges：为异构图添加反向边，增强图完整性。
# - EarlyStopping：实现早停机制，保存最佳模型和损失/分数。
# - get_nodes_dict：获取每个节点类型的节点数。
# - extract_embed：从输入节点提取嵌入。
# - set_random_seed：设置随机种子，确保实验可重复。
# - com_mult/conj/ccorr：支持复杂数运算，用于特征变换。
# - transform_relation_graph_list：提取特定关系子图。
# - extract_mtx_with_id_edge：生成带边类型标识的邻接矩阵。
# - h2dict/print_dict：处理和打印字典数据。
# - extract_metapaths：提取元路径字典。
# - to_hetero_feat/to_hetero_idx：将同构特征/索引转为异构格式。
# - get_ntypes_from_canonical_etypes：从边类型提取节点类型。
# - scatter_*：实现散点聚合操作（如 sum、mean），支持图消息传递。
...
def parse_fanouts(s):
    try:
        ...
        raise argparse.ArgumentTypeError(
            "dataset-fanouts must be a comma-separated list of integers"
        )